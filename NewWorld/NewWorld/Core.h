...
public:
    ...
    void init(HWND hwnd, int _width, int _height)
    {
        ...
        //Create a first root signature that allows drawing
        // Allows drawing
    ...
    void uploadResource(ID3D12Resource* dstResource, const void* data, unsigned int size,
        ...
        ID3D12Resource* uploadBuffer;//GPU buffer

        D3D12_HEAP_PROPERTIES heapProps = {};
        heapProps.Type = D3D12_HEAP_TYPE_UPLOAD;//ÅäÖÃ Upload Heap ÊôÐÔ

        D3D12_RESOURCE_DESC bufferDesc = {};//resource description
